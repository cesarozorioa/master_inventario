...
const Devolucion = () => {
 ...
 const [tiposProducto, setTiposProducto] = useState([]);
  ...
  useEffect(() => {
    ...
    axios
      .get("http://127.0.0.1:8000/api/v1/devolucion")
    ...
    axios
      .get("http://127.0.0.1:8000/api/v1/sucursal/")
  ...
  const guardarDevolucion = () => {
    const nuevoDevolucion = {
      ...
      idSuc_fk: selectedSucursal.idSucursal,
    ...
    if (isEditing) {
      axios
        .put(
          `http://127.0.0.1:8000/api/v1/devolucion/${editingDevolucion.idDevolucion}/`,
          nuevoDevolucion
        )
        .then((response) => {
          setDevoluciones(
      ...
      axios
        .post("http://127.0.0.1:8000/api/v1/devolucion/", nuevoDevolucion)
        .then((response) => {
  ...
  const eliminarDevolucion = (devolucionId) => {
    axios
      .delete(`http://127.0.0.1:8000/api/v1/devolucion/${devolucionId}/`)
      .then(() => {
        setDevoluciones(
          devoluciones.filter(
            (devolucion) => devolucion.idDevolucion !== devolucionId
          )
        );
      })
      .catch((error) => {
  ...
  const accionPlantilla = (rowData) => (
    <React.Fragment>
      <Button
        label="Editar"
        onClick={() => editarDevolucion(rowData)}
        className="p-button-warning mr-2"
        icon="pi pi-pencil"
      />
      <Button
        label="Eliminar"
        onClick={() => eliminarDevolucion(rowData.idDevolucion)}
  ...
  const renderModal = () => (
    ...
    >
      ...
      <div className="p-field devolucion_medida"  >
        ...
        {selectedProducto && (
          <div className="p-field">
            <label>{selectedProducto.unidadProducto}</label>
          </div>
        )}
        {/*Unidad de Medida*/}
      </div>
      ...
      <Button 
        ...
        className="p-button-success mt-3 boton_devolucion"
        icon="pi pi-check"        
