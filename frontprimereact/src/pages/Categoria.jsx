...
const Categoria = () => {
  ...
  const obtenerCategorias = async () => {
    try {
      ...
      toast.current.show({ severity: "error", summary: "Error", detail: "No se pudo cargar las categorias" });
    }
  ...
  const guardarCategoria = async () => {
    ...
    if (isEditing) {
      ...
      try {
        await axios.put(`http://127.0.0.1:8000/api/v1/categoria/${idCategoria}/`,{ nombCategoria:nombre});
        toast.current.show({ severity: "success", summary: "Éxito", detail: "Categoria modificado correctamente" });
      } catch (error) {
        console.log(error)
        toast.current.show({ severity: "error", summary: "Error", detail: "No se pudo modificar Categoria" });
      }
    } else {
      try {
        await axios.post("http://127.0.0.1:8000/api/v1/categoria/", { nombCategoria:nombre});        
        toast.current.show({ severity: "success", summary: "Éxito", detail: "Tipo de categoria agregado correctamente" });
      } catch (error) {
        console.log(error)
  ...
  const eliminarCategoria = async (categoriaId) => {  
    ...
    try {
      await axios.delete(`http://127.0.0.1:8000/api/v1/categoria/${categoriaId}/`);
      obtenerCategorias();
      toast.current.show({ severity: "success", summary: "Éxito", detail: "Categoria eliminado correctamente" });
    } catch (error) {
      console.log(error)
      toast.current.show({ severity: "error", summary: "Error", detail: "No se pudo eliminar La categoria" });
    }
  };
  ...
  const accionPlantilla = (rowData) => {    
    return (
      <div>
        ...
        <Button label="Eliminar" icon="pi pi-trash" onClick={() => eliminarCategoria(rowData.idCategoria)} className="p-button-danger" />
      </div>
    );
  };
  ...
  return (
    ...
    <div className="p-d-flex p-jc-end p-mb-3">
      <Button label="Agregar Categoria" icon="pi pi-plus" onClick={() => abrirDialogo(false)} className="p-button-success" />
    </div>
    ...
    >
      ...
      <div className="p-d-flex p-jc-between p-mt-4">
        ...
        <Button label="Guardar" icon="pi pi-check" className="p-button-primary" onClick={guardarCategoria} />
      </div>
    </Dialog>
  </div> 
  )
